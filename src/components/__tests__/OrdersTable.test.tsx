...
describe("OrdersTable - MIG-01 Design System Migration", () => {
  ...
  describe("STATUS_PILL_CONFIG uses design tokens (no hardcoded hex)", () => {
    it("does not use hardcoded #f59e0b22 (old Producing countBg)", async () => {
    ...
    it("does not use hardcoded #2b6cb022 (old Ready countBg)", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("does not use hardcoded #9333ea22 (old In Transit countBg)", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("does not use hardcoded #2f855a22 (old Complete countBg)", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("does not use hardcoded bg-gray-100", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("does not use hardcoded text-gray-600", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
  ...
  describe("FilterPill import from ui/ directory", () => {
    it("renders FilterPill components for status filters", async () => {
      render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("FilterPill uses token-based countBg with var(--status-mixing-bg-22)", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
  ...
  describe("Search and hover states use tokens", () => {
    it("search icon uses text-[var(--text-secondary)]", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
    ...
    it("uses token-based hover:bg-[var(--bg-page)]", async () => {
      const { container } = render(
        <OrdersTable
          selectedOrderId={null}
          onSelectOrder={jest.fn()}
