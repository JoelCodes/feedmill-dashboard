...
| `src/actions/import.ts` (`previewImportAction`) | n/a (new server action, Phase 33) | Server action (`'use server'`) | n/a | `previewImportAction` reads production_orders via a single SELECT (no mutations — pure preview) | `await requireRole('mill_operator')` — first line of action body (AUTH-02) | 3-layer file-size DoS mitigation: layer 1 = Phase 34 client `<input>` size check using exported `MAX_IMPORT_BYTES`; layer 2 = `file.size > MAX_IMPORT_BYTES` in action body (T-33-DoS); layer 3 = Next.js 2 MB framework body-size limit (plan 33-01 `next.config.ts` `experimental.serverActions.bodySizeLimit`). xlsx/SheetJS banned (CVE-2023-30533); `read-excel-file 9.0.9` used instead (T-33-XLSX). |
