...
**Note:** Do NOT use the whole-object form `{"metadata": "{{user.public_metadata}}"}`. That shape (a) can exceed Clerk's 1.2 KB custom-claim ceiling for accounts that accrue arbitrary `publicMetadata` fields, and (b) does not match the `CustomJwtSessionClaims.metadata.role` declaration in `src/types/clerk.d.ts`. The single-field template above selects only the `role` value and keeps the token small.
...
**Note:** The `e2e-norole` row deliberately has no role assigned. This user proves the redirect-to-root behavior — an authenticated user with no `publicMetadata.role` who tries `/demo/*` must be redirected to `/`.

## Step 3: Assign publicMetadata.role
...
**Note:** Leave the `e2e-norole+clerk_test@example.com` user with no `publicMetadata.role` set. Do not write `{"role": null}` or `{"role": ""}` — leave the field absent entirely.
...
3. Open browser devtools, copy the `__session` cookie value, and decode it at jwt.io. Confirm the payload contains `"metadata": {"role": "demo"}` for the demo user and `"metadata": {"role": "admin"}` for the admin user. The norole user's decoded token should have `metadata` absent or `metadata.role` absent.
...
Deviating from this order leaves the user's session cookie stuck with a stale or missing `metadata.role` claim, which presents as "demo user is redirected to /" or "decoded JWT has no metadata field". Cross-reference the Phase 27 RESEARCH document Pitfall 2 if you hit this symptom.
