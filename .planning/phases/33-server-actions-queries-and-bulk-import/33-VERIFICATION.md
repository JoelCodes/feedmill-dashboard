...
gaps:
  ...
  - id: GAP-03
    ...
    status: "CLOSED by plan 33-11 (Task 3 commit eb97785). Harness re-ran end-to-end against live Neon dev DB after GAP-05 API migration: 31 rows committed, assertPostConditions PASSED, PASS: GAP-03 closed. See 33-07-SUMMARY.md 2026-05-14 Addendum."
  - id: GAP-04
    ...
    status: "CLOSED by plan 33-10 (commits c22badb/b8c02c8/fa6f0c5/40bd437/3e5adb7). Guard is correct but insufficient — the deeper API mismatch is captured separately as GAP-05."
  - id: GAP-05
    truth: "previewImportAction and commitImportAction successfully parse a real Book1.xlsx file against read-excel-file v9.0.9 (the pinned dependency)"
    why_gap: "Surfaced 2026-05-14 by the GAP-03 harness after GAP-04's `?? []` guard let the code reach the next crash point at `rawRows.length` (undefined). Root cause: src/actions/import.ts:133 calls `readXlsxFile(buffer, { schema })` with `import readXlsxFile from 'read-excel-file/node'` (the default export). In read-excel-file v9.x the `schema` parameter was REMOVED from `readXlsxFile` and re-added only to the named export `readSheet`. Per node_modules/read-excel-file/node/index.d.ts: `readXlsxFile(input, options?): Promise<Sheet<ParsedNumber>[]>` — returns a 2D Sheet array, NOT `{ rows, errors }`. The schema option is silently ignored at runtime. Per node_modules/read-excel-file/types/parseSheetData/parseSheetData.d.ts: the schema-parsed return shape is `{ objects, errors }` (note: `objects` not `rows`), available only via `readSheet(input, { schema })` or `parseData(sheet, schema)`. Per node_modules/read-excel-file/README.md migration notes: 'Old: const { rows, errors } = await readXlsxFile(..., { schema }) — Removed schema parameter. Re-added schema parameter to readSheet() function: const { objects, errors } = readSheet(data, { schema })'. The Phase 33 unit tests (import-preview.test.ts, import-commit.test.ts) mock readXlsxFile with `{ rows: [...], errors: [...] }` — a shape the real v9.x library does not return. The mocks lock in a fictional API and have been validating contracts against that fictional shape since 33-05/33-06 landed. Production impact: every XLSX upload to previewImportAction would crash at rawRows.length before parsing anything; commitImportAction's re-parse path would crash identically."
    fix_hint: "Migrate src/actions/import.ts: replace `import readXlsxFile from 'read-excel-file/node'` with `import { readSheet } from 'read-excel-file/node'`. Replace the single call site `readXlsxFile(buffer, { schema: xlsxSchema })` with `readSheet(buffer, { schema: xlsxSchema })`. Update the XlsxFn type cast: rename `rows: Record<string, unknown>[]` to `objects: Record<string, unknown>[]` (the v9.x return key) — the destructure can keep `objects: rawRows` to preserve the internal `rawRows` name. Update unit-test mocks in import-preview.test.ts and import-commit.test.ts: every `mockResolvedValue({ rows, errors })` becomes `mockResolvedValue({ objects, errors })`. Apply the same migration to scripts/test-xlsx-import.ts (33-07's harness duplicates the same call pattern). Re-run `npm test` and `npm run test:xlsx-import` to confirm. After this lands, GAP-03 Task 3 (the operator harness re-run) should pass."
    status: "CLOSED by plan 33-11 (commits d015697/58f4f1a/eb97785/44cfdb4/afbe118). readSheet API migration complete: import.ts and harness now use readSheet with {objects,errors}. Additional Rule 1 fixes applied: removed required:true from xlsxSchema (v9.x returns error-branch for ALL rows when any required field missing; Zod handles required validation), added null-row guard for empty XLSX rows. Unit tests: 50/50 pass. Live-DB harness: PASS. See 33-11-SUMMARY.md."
human_verification:
deferred:
...
**Verified:** 2026-05-13T00:00:00Z (initial); 2026-05-14T05:00:00Z (GAP-04 appended); 2026-05-14T05:30:00Z (GAP-05 appended after GAP-04 fix exposed deeper API mismatch)
**Status:** gaps_found (3 initial gaps + 2 follow-up code bugs surfaced by GAP-03 harness)
...
| GAP-04 (CLOSED) | previewImportAction and commitImportAction handle clean files (zero parser errors) without crashing | Surfaced by GAP-03 harness 2026-05-14. `for (const pe of parserErrors)` throws when read-excel-file v9.0.9 returns `errors: undefined` for clean files. | Closed by plan 33-10. Guard `?? []` added; tests cover the undefined-errors path. Insufficient on its own — see GAP-05. |
| GAP-05 | previewImportAction and commitImportAction successfully parse a real Book1.xlsx file against read-excel-file v9.0.9 | Surfaced by GAP-03 harness after GAP-04 fix landed. `readXlsxFile(buf, {schema})` no longer accepts `schema` in v9.x — the option is silently ignored and the function returns `Sheet[]`, not `{ rows, errors }`. Schema-based parsing moved to `readSheet(buf, {schema})` which returns `{ objects, errors }` (note: `objects` not `rows`). Unit tests mock a v8.x-shaped return that doesn't match the real v9.x library. | Migrate `import readXlsxFile from 'read-excel-file/node'` → `import { readSheet } from 'read-excel-file/node'`. Update the destructure to `objects: rawRows`. Update unit-test mocks to return `{ objects, errors }`. Apply same migration to scripts/test-xlsx-import.ts. |
**Re-verification:** Partial — GAP-04 closed by 33-10; GAP-05 appended after the 33-10 fix exposed the underlying API mismatch
