...
reasoning_checkpoint:
  hypothesis: "The Phase 31-05 fix `@source not \"../../.planning/**/*\"` uses an unsupported glob form. Tailwind v4's `@source not` accepts ONLY a directory path (e.g. `@source not \"../../.planning\"`), not file-level glob patterns. As a result the directive is silently a no-op, Oxide's default scanner walks `.planning/**/*.md` (since .planning is NOT .gitignored), discovers the literal `text-[var(--text-*)]` in five markdown files, and emits a malformed CSS rule `.text-\\[var\\(--text-\\*\\)\\] { color: var(--text-*); }` that LightningCSS rejects with `Unexpected token Delim('*')`."
  confirming_evidence:
